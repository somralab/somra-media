...
const Implementation = "torznab"

// Factory builds Torznab indexer plugins.
type Factory struct{}

// NewFactory returns a Torznab factory.
...
func (Factory) New(_ context.Context, instanceID string, config []byte) (plugin.Plugin, error) {
	...
	if len(config) > 0 {
		if err := json.Unmarshal(config, &cfg); err != nil {
			return nil, fmt.Errorf("torznab config: %w", err)
		}
	}
	apiKey, _ := configSecret(config, "apiKey")
	client, err := nzbindexer.NewClient(cfg, apiKey, nzbindexer.ProtocolTorrent)
	if err != nil {
		return nil, err
	}
