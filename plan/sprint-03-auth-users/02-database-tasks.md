...
- [ ] A1 — `user`, `role`, `permission`, `user_role` tabloları | Kabul: migrasyon + bütünlük.
- [ ] A2 — `session`/token tablosu (çoklu cihaz) | Kabul: süre/iptal alanları.

### Epik B: Profil ve kontrol
- [ ] B1 — `user_profile` (tercih, **dil/locale**, **arayüz teması**, avatar), ebeveyn kontrolü alanları (rating sınırı) | Kabul: çocuk profili kısıtları sorgulanabilir; dil ve tema tercihi saklanır (tema varsayılanı `cinematic`). Bkz. [`../i18n-localization.md`](../i18n-localization.md).

### Epik C: İzleme durumu
- [ ] C1 — `watch_state` (ilerleme, izlendi), `favorite`, `watchlist` tabloları | Kabul: kullanıcı+öğe bazlı indeksler.
