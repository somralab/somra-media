...
- [ ] A1 — SQLite bağlantı yönetimi (WAL modu, bağlantı havuzu, pragma ayarları) | Kabul: WAL aktif, eşzamanlı okuma testi geçer.
- [ ] A2 — Repository/erişim deseni iskeleti | Kabul: örnek tablo için CRUD testli.
- [ ] A3 — İşlem (transaction) yardımcıları | Kabul: rollback/commit testleri.

### Epik B: Migrasyon ve şema sürümleme
- [ ] B1 — Migrasyon altyapısı (ileri/geri) kurulumu | Kabul: `up`/`down` çalışır, sürüm tablosu tutulur.
- [ ] B2 — Şema sürümleme ve uygulama açılışında otomatik migrasyon | Kabul: yükseltmede şema otomatik güncellenir.
- [ ] B3 — Tohum (seed) ve test veri altyapısı | Kabul: test DB izole kurulur.

### Epik C: Yedekleme/dayanıklılık temeli
- [ ] C1 — DB dosyası konumu ve volume stratejisi (kalıcılık) | Kabul: [`05-devops-tasks.md`](./05-devops-tasks.md) ile uyumlu volume.
- [ ] C2 — Bütünlük kontrolü ve bozulma kurtarma notları | Kabul: temel `PRAGMA integrity_check` akışı.
